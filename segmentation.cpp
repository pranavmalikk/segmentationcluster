...
//     std::vector<Component>& tempComponents,
//     std::unordered_set<Triangle>& remainingTriangles,
//     std::unordered_set<Triangle>& skippedTriangles,
//     std::unordered_set<Triangle>& usedTriangles,
//     std::vector<int>& repetitionCounts,
//     std::vector<double>& componentWeights,
//     std::vector<double>& globalVolumeChangeHistory,
...
//                     skippedTriangles, usedTriangles,
    ...
    while (true) {
        ...
        for (size_t i = 0; i < tempComponents.size(); ++i) {
            ...
            if (isValidAfterAddingTriangle(tempComponents[i], bestTriangle) && 
                ...
                badlyGrownTempComponents.push_back(tempComponents[i]);
                componentIndexStack.push(i);
                skippedTriangles.insert(bestTriangle);
                componentsStalled[i] = true;
        ...
        for (size_t i = 0; i < tempComponents.size(); ++i) {
            if (componentsStalled[i]) {
                ...
                std::cout << "Skipped triangles for component " << i << ": " << skippedTriangles.size() << std::endl;
            }
        }
        ...
        if (anyComponentStalled) {
            // Use the global stack for backtracking
            // globalBacktrackStack.push(grownTempComponents);
            unhappyBacktrackStack.push(badlyGrownTempComponents);
            // std::cout << "-----GLOBAL BACKTRACK STACK-----" << std::endl;
            // inspectStack(globalBacktrackStack);
            // std::cout << "-----UNHAPPY BACKTRACK STACK-----" << std::endl;
            // inspectStack(unhappyBacktrackStack);
            std::cout << "-----BACKTRACKING NOW-----" << std::endl;
                bool result = recursiveBackTracking(
                    ...
                    componentIndexStack, tempComponents, remainingTriangles, 
                    skippedTriangles, usedTriangles,
                    repetitionCounts, componentWeights, globalVolumeChangeHistory, 0
                );
...
std::vector<Component> randomizedGrowthOptimization(const Cluster& cluster,
    ...
    while (!remainingTriangles.empty()) {
        ...
        while (canGrow) {
            ...
            std::cout << "Triangle count before growing in skippedTriangles: " << skippedTriangles.size() << std::endl;
            ...
            canGrow = growComponentsSynchronously(
                ...
                skippedTriangles, typeCounts, usedTriangles
            );
            ...
            std::cout << "Triangle count after growing in skippedTriangles: " << skippedTriangles.size() << std::endl;
...
void refineClusters(std::vector<Cluster>& clusters, double tau_N, double spatialHashCellSize) {
    ...
    while (true) {
        ...
        for (size_t i = 0; i < clusters.size(); ++i) {
            ...
            std::unordered_set<int> comparedClusters;


            for (size_t idx = 0; idx < neighboringClusterIndicesVec.size(); ++idx) {
                int j = neighboringClusterIndicesVec[idx];
                if (i == j || clusters[j].triangles.empty() || comparedClusters.count(j) > 0) continue;
                ...
                double similarity = ATaTb(clusters[i], clusters[j], potentialNeighborsCache, triangleAdjacency, tbTriangles, tau_N);
                // std::cout << "Similarity between cluster " << i << " and cluster " << j << ": " << similarity << std::endl;
                std::unordered_set<Triangle> toBeMerged;
                std::unordered_set<Triangle> toBeSplit;

                if (similarity >= tau_N) {
                    comparedClusters.insert(i);
                    comparedClusters.insert(j);
                    for (const auto& triangleA : clusters[i].triangles) {
                    ...
                    if (!toBeMerged.empty()) {
                        for (const auto& triangle : toBeMerged) {
                            if (alreadyAddedToMaps.count(triangle) == 0) {
                                // std::cout << "Adding triangle to be merged: " << triangle.toString() << std::endl;
                    ...
                    if (!toBeSplit.empty()) {
                        for (const auto& triangle : toBeSplit) {
                            if (alreadyAddedToMaps.count(triangle) == 0) {
                                // std::cout << "Adding triangle to be split: " << triangle.toString() << std::endl;
            ...
            // std::cout << "Content of alreadyAddedToMaps for cluster " << i << ": " << alreadyAddedToMaps.size();
            // std::cout << std::endl;
       ...
       // Your existing random generator and other variables remain unchanged
        ...
        for (auto& entry : splitMap) {
            ...
            // std::cout << "Original cluster index: " << originalClusterIndex << std::endl;
            auto& originalClusterTriangles = clusters[originalClusterIndex].triangles;

            for (const auto& triangle : trianglesToSplit) {
                // std::cout << "Removing triangle: " << triangle.toString() << " from cluster: " << originalClusterIndex << std::endl;
                originalClusterTriangles.erase(
                    std::remove(originalClusterTriangles.begin(), originalClusterTriangles.end(), triangle),
                    originalClusterTriangles.end());
            }
            ...
            Cluster newCluster;
            for (const auto& triangle : trianglesToSplit) {
                if (alreadyAddedToMaps.count(triangle) > 0) continue;  // Use the original alreadyAddedToMaps here

                newCluster.triangles.push_back(triangle);
            }

            // std::cout << "New cluster size: " << newCluster.triangles.size() << std::endl;
            clusters.push_back(newCluster);

            for (const auto& triangle : trianglesToSplit) {
                if (alreadyAddedToMaps.count(triangle) > 0) continue;  // Use the original alreadyAddedToMaps here

                originalClusterOfTriangle[triangle] = clusters.size() - 1;
            }
        }
